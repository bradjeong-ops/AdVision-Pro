...
export default function GuestLoginModal({ onLogin, onClose, requireApiKey = false, initialPin = '' }: GuestLoginModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="relative bg-[#1a2235] border border-white/10 p-8 rounded-[32px] shadow-2xl w-full max-w-md flex flex-col items-center text-center animate-in zoom-in-95 duration-300">
        ...
        <h2 className="text-2xl font-black text-white mb-2 tracking-tight">GUEST ACCESS</h2>
        <p className="text-sm text-slate-400 mb-8 leading-relaxed">
          Enter a 4-digit PIN {requireApiKey && "and your Gemini API Key "}to access your workspace.
        </p>
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
          ...
          {requireApiKey && (
            <div className="w-full text-left mt-2">
              ...
              <input
                ...
                placeholder="AIzaSy..."
                className="w-full bg-black/50 border border-white/10 rounded-2xl px-6 py-4 text-sm font-mono text-white focus:outline-none focus:border-indigo-500/50 transition-colors placeholder:text-white/20"
              />
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={pin.length !== 4 || (requireApiKey && !apiKey.trim())}
            className="w-full mt-4 bg-indigo-600 hover:bg-indigo-500 disabled:bg-white/5 disabled:text-white/30 text-white font-black py-4 rounded-2xl transition-all flex items-center justify-center gap-2 group"
          >
            ACCESS WORKSPACE
